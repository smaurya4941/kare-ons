    ...
    @if($wishlists->isEmpty())
        <div class="bg-surface rounded-xl border border-outline-variant shadow-sm p-10 text-center">
            <div class="w-16 h-16 bg-herbal-light mx-auto rounded-full flex items-center justify-center mb-5">
                <span class="material-symbols-outlined text-herbal-accent text-3xl">favorite</span>
            </div>
            <h3 class="text-xl font-bold text-on-surface mb-2">Your wishlist is empty</h3>
            <p class="text-sm text-on-surface-variant mb-6">Save items you love to your wishlist and review them later.</p>
    ...
    @else
        <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
            @foreach($wishlists as $item)
                <div class="bg-surface rounded-xl border border-soft-border overflow-hidden hover:shadow-md transition-shadow group flex flex-col">
                        ...
                        <div class="mt-auto">
                            <div class="flex items-center gap-2 mb-4">
                                @if($item->product->sale_price)
                                    ...
                                    <span class="text-sm text-on-surface-variant line-through">₹{{ number_format($item->product->price, 2) }}</span>
                                @else
                            ...
                            <form action="{{ route('cart.add') }}" method="POST">
                                ...
                                <button type="submit" class="w-full flex items-center justify-center gap-2 bg-primary-container text-on-primary-container hover:bg-primary hover:text-white font-medium py-2.5 rounded-lg transition text-sm">
