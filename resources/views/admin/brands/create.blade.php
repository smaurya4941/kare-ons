...
<form action="{{ route('admin.brands.store') }}" method="POST" enctype="multipart/form-data" class="space-y-4 max-w-3xl">
    @csrf
        ...
        <div class="space-y-4">
            ...
            <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
                <button type="submit" class="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-medium py-2 px-4 rounded-md transition shadow-sm flex justify-center items-center gap-1.5 text-[11px]">
                    <span class="material-symbols-outlined text-[16px]">save</span> Save Brand
                </button>
            </div>
        </div>
    </div>
